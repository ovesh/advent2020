...
func (c validCache) memoize(s string, isValid bool) {
	if isValid {
		c.valid[s] = true
	} else {
		c.invalid[s] = true
...
func (c validCache) cached(s string) bool {
	return c.valid[s] || c.invalid[s]
}
func (c validCache) isValid(s string) bool {
	return c.valid[s]
}
...
func (r stringRule) validate(s string) bool {
	if r.validCache.cached(s) {
		return r.validCache.isValid(s)
	}
	res := r.s == s
	r.validCache.memoize(s, res)
	return res
}
...
func (r orRule) validate(s string) bool {
	if r.validCache.cached(s) {
		return r.validCache.isValid(s)
	}
	res := r.rule1.validate(s) || r.rule2.validate(s)
	r.validCache.memoize(s, res)
	return res
}
...
func (r concatRule) validate(s string) bool {
	if r.validCache.cached(s) {
		return r.validCache.isValid(s)
	}
	res := false
	for i := 0; i < len(s); i++ {
		if r.rule1.validate(s[0:i]) && r.rule2.validate(s[i:]) {
			res = true
			break
		}
	}
...
func (r identityRule) validate(s string) bool {
	if r.validCache.cached(s) {
		return r.validCache.isValid(s)
	}
	res := rulesByID[r.ruleID].validate(s)
	r.validCache.memoize(s, res)
	return res
}

var concatRuleRE = regexp.MustCompile("[0-9]+ [0-9]")
var stringRuleRE = regexp.MustCompile("\".*\"")

var rulesByID = map[int]rule{}
...
func parseRule(s string) rule {
	...
	if strings.Contains(s, " | ") {
		...
		return orRule{
			validCache: newValidCache(),
			rule1:      parseRule(split[0]),
			rule2:      parseRule(split[1]),
		}
	}
	if concatRuleRE.Match([]byte(s)) {
		split := strings.Split(s, " ")
		if len(split) > 2 {
			panic(fmt.Sprintf("can't handle %d rules: %q", len(split), s))
		}
		return concatRule{
			validCache: newValidCache(),
			rule1:      parseRule(split[0]),
	...
	// must be an identity rule
	ruleID, err := strconv.Atoi(s)
	if err != nil {
...
func CountValidMessages() {
	parts := strings.Split(input, "\n\n")
	ruleLines := strings.Split(parts[0], "\n")
	for _, line := range ruleLines {
		split := strings.Split(line, ": ")
		ruleID, err := strconv.Atoi(split[0])
		if err != nil {
			panic(fmt.Sprintf("invalid rule id %q", split[0]))
		}
		rulesByID[ruleID] = parseRule(split[1])
	}

	msgLines := strings.Split(parts[1], "\n")
	rule0 := rulesByID[0]
	matches := 0
	for _, line := range msgLines {
		if rule0.validate(line) {
			matches++
		}
	}

	fmt.Println("total messages matching rule 0: ", matches)
}
