...
const passportsRaw = `pid:827837505 byr:1976
hgt:187cm
iyr:2016
hcl:#fffffd
eyr:2024
...
func loadPassportDB() []passport {
	rawPassports := strings.Split(passportsRaw, "\n\n")
	res := make([]passport, len(rawPassports))
	for i, rawP := range rawPassports {
		newlinesReplaced := strings.Replace(rawP, "\n", " ", -1)
		split := strings.Split(newlinesReplaced, " ")
		p := passport{}
		for _, field := range split {
			keyVal := strings.Split(field, ":")
			if len(keyVal) != 2 {
				fmt.Println("got this: ", rawP)
				panic("baaaad")
			}
			switch keyVal[0] {
			case "byr":
