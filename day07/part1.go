...
const colorRuleInput = `muted lime bags contain 1 wavy lime bag, 1 vibrant green bag, 3 light yellow bags.
...
type rule struct {
	outerColor      string
	colorsToAmounts map[string]int
}

//var bagRuleRE = regexp.MustCompile("^(\\d+) ([a-z]+ [a-z]+) bags$")

func loadLuggageRules() []rule {
	lines := strings.Split(colorRuleInput, "\n")
	res := make([]rule, len(lines))
	for _, line := range lines {
		rule := rule{colorsToAmounts: map[string]int{}}
		split := strings.Split(line, " bags contain ")
		outerColor := split[0]
		rule.outerColor = outerColor
		res = append(res, rule)
		if split[1] == "no other bags." {
			continue
		}
		// remove period
		curRulesRaw := strings.Split(split[1][:len(split[1])-1], ", ")
		for _, ruleRaw := range curRulesRaw {
			splitRule := strings.Split(ruleRaw, " ")
			numBags, err := strconv.Atoi(splitRule[0])
			if err != nil {
				panic("failed to parse: " + line)
			}
			internalColor := splitRule[1] + " " + splitRule[2]
			rule.colorsToAmounts[internalColor] = numBags
		}
	}
...
func colorToAllowedContainers(rules []rule) map[string][]string {
	res := make(map[string][]string, len(rules))
	for _, rule := range rules {
		for containableColor := range rule.colorsToAmounts {
			//if _, ok := res[containableColor]; !ok {
			//	res[containableColor] = []string{}
			//}
			res[containableColor] = append(res[containableColor], rule.outerColor)
		}
	}
	return res
}

func getPossibleContainersFor(color string, colorToAllowedContainers map[string][]string) []string {
	res := colorToAllowedContainers[color]
	fmt.Printf("direct containers for %s: %#v\n", color, res)
	for _, indirectColor := range res {
		containersFor := getPossibleContainersFor(indirectColor, colorToAllowedContainers)
		fmt.Printf("containers for %s: %#v\n", indirectColor, containersFor)
		res = append(res, containersFor...)
	}
	return res
}

func MayContainBag(color string) {
	rules := loadLuggageRules()
	//fmt.Printf("%#v", rules)

	inverted := colorToAllowedContainers(rules)
	//fmt.Printf("%#v", inverted)

	containersFor := getPossibleContainersFor(color, inverted)
	deduped := map[string]bool{}
	for _, clr := range containersFor {
		deduped[clr] = true
	}
	fmt.Println(len(containersFor))
