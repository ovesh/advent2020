...
func allAdjacentSeatsAreEmpty(state []string, row, col int) bool {
	// up
	if row > 0 && string(state[row-1][col]) == "#" {
		return false
	}
	// up left
	if row > 0 && col > 0 && string(state[row-1][col-1]) == "#" {
		return false
	}
	// left
	if col > 0 && string(state[row][col-1]) == "#" {
		return false
	}
	numRows := len(state)
	// left down
	if row < numRows-1 && col > 0 && string(state[row+1][col-1]) == "#" {
		return false
	}
	// down
	if row < numRows-1 && string(state[row+1][col]) == "#" {
		return false
	}
	rowLen := len(state[0])
	// right down
	if row < numRows-1 && col < rowLen-1 && string(state[row+1][col+1]) == "#" {
		return false
	}
	// right
	if col < rowLen-1 && string(state[row][col+1]) == "#" {
		return false
	}
	// right up
	if row > 0 && col < rowLen-1 && string(state[row-1][col+1]) == "#" {
		return false
	}
...
func countOccupiedAdjacentSeats(state []string, row, col int) int {
	count := 0
	// up
	if row > 0 && string(state[row-1][col]) == "#" {
		count++
...
func stepSeatSimulation(initialState []string) []string {
	...
	for row := range initialState {
		for col := range initialState[row] {
			...
			if allAdjacentSeatsAreEmpty(initialState, row, col) {
				res[row] = res[row][0:col] + "#" + res[row][col+1:]
			}
			if string(initialState[row][col]) == "#" && countOccupiedAdjacentSeats(initialState, row, col) > 3 {
...
func seatStatesEqual(before, after []string) bool {
	for i := range before {
		if before[i] != after[i] {
			return false
		}
...
func HowManyEmptySeats() {
	for {
		afterStep := stepSeatSimulation(seatsInitialState)
