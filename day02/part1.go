...
const pwdb = `1-13 r: gqdrspndrpsrjfjx
...
func loadDb() []record {
	rawRecords := strings.Split(pwdb, "\n")
	records := make([]record, len(rawRecords))
	for i, rawRecord := range rawRecords {
		split := strings.Split(rawRecord, " ")
		rawMinMax := strings.Split(split[0], "-")
		minTimes, _ := strconv.Atoi(rawMinMax[0])
		maxTimes, _ := strconv.Atoi(rawMinMax[1])
		records[i] = record{
			minTimes: minTimes,
			maxTimes: maxTimes,
			char:     rune(split[1][0]),
...
func isValid(record record) bool {
	count := 0
	for _, c := range record.pwd {
		if c == record.char {
			count++
		}
	}
	return count <= record.maxTimes && count >= record.minTimes
}
